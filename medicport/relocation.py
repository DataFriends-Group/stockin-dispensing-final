...
def relocate_item(
    ...
    if is_negative_z:
        if has_existing_items:
            new_z_end = new_vsu.position.z - item_depth - DEPTH_GAP_BETWEEN_ITEMS
            new_z_start = new_z_end - item_depth
        else:
            back_wall_z = new_vsu.position.z - new_vsu.dimensions.depth
            new_z_start = back_wall_z
            new_z_end = back_wall_z + item_depth
    else:
        if has_existing_items:
            new_z_start = new_vsu.position.z + item_depth + DEPTH_GAP_BETWEEN_ITEMS
            new_z_end = new_z_start + item_depth
