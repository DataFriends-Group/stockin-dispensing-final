...
VSU_TOP_CLEARANCE = 5  # Gap from item top to shelf
...
def calculate_stock_index(vsu: VirtualStorageUnit, item_depth: float, is_new_vsu: bool, is_mixed_product: bool = False) -> int:
    ...
    for item_id in vsu.items:
        taken.add(items[item_id].stock_index)
    for p in pending_placements.values():
        if p["vsu_id"] == vsu.id:
            taken.add(p["stock_index"])

    for idx in range(max_items - 1, -1, -1):
        if idx not in taken:
            return idx
