...
the buttons act on the warehouse main.py actually has in memory.

Usage:
    python3 warehouse_view_robot.py --input ../data/R3.json --output ../reports/warehouse_view_robot
...
def item_tooltip(item):
    lines = [
        ...
        f"Rozmery: {item.width:.0f} x {item.height:.0f} x {item.depth:.0f} mm",
    ]
    if item.batch:
        lines.append(f"Sarze: {item.batch}")
    ...
    for vsu in shelf.vsus:
        ...
        for item in vsu.items:
            ...
            rows.append(
                ...
                f"<td>{html.escape(format_expiration(item.expiration))}</td>{robot_cells}</tr>"
            )
        if not vsu.items:
            rows.append(
