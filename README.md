...
| Soubor | Popis |
|---|---|
| `R3.json` | Aktuální stav skladu – hlavní inventář (items, VSU, shelves, racks). Cesta se nastavuje v `config.py` (`INVENTORY_FILENAME`) |
...
- `data/R3.json` — musí obsahovat layout skladu (racks, shelves, VSUs, items); jiný soubor lze nastavit přes `INVENTORY_FILENAME` v `config.py`
...
- `GET /warehouse/all` — Obsah celého skladu agregovaný podle barcode (jen potvrzené položky umístěné ve VSU) — `[{"Id": barcode, "Quantity": počet}]`
...
- Inventář se drží v paměti (načteno při startu z `INVENTORY_FILE`, viz `config.py` — aktuálně `data/R3.json`), změny se zapisují do stejného souboru po každém complete/fail
